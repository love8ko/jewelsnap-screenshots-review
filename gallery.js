(async()=>{
 ...
 try{const r=await fetch('gallery.json', {cache:'no-store'});if(!r.ok)throw Error('load');data=await r.json();}catch{$('#gallery').textContent='Не удалось загрузить галерею. Обновите страницу.';return;}
 const valid=id=>/^[MVONR]0[1-8]$/.test(id);
 const original=id=>`originals/${id}.${id.startsWith('O')?'jpg':'png'}?v=${encodeURIComponent(data.version)}`;
 ...
 for(const series of data.series){
  ...
  for(const id of series.frames){if(!valid(id))continue;const f=document.createElement('figure');const cap=document.createElement('figcaption');cap.textContent=id;const button=document.createElement('button');button.className='shot';button.dataset.id=id;button.setAttribute('aria-label',`Увеличить ${id}`);const img=document.createElement('img');img.src=`previews/${id}.jpg?v=${encodeURIComponent(data.version)}`;img.alt=`${id} — ${series.name}`;img.width=440;img.height=956;img.loading=id.startsWith('M')?'eager':'lazy';button.append(img);f.append(cap,button);row.append(f);
   button.addEventListener('click',()=>{const url=original(id);$('#viewer-title').textContent=id;$('#image-error').hidden=true;$('#large').hidden=false;$('#large').alt=`${id} — полный размер`;$('#large').src=url;$('#download').href=url;$('#download').download=url.split('/').pop().split('?')[0];$('#full').href=url;$('#retry-original').href=url;$('#viewer').showModal();});
  }
